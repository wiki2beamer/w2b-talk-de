\documentclass[ hyperref={colorlinks=true,filecolor=blue,linkcolor=blue,urlcolor=blue}]{beamer}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{color}
\usepackage{textcomp}
\usepackage{graphicx}
\usepackage{lmodern}

\usepackage{beamerthemesplit}


\setbeamertemplate{navigation symbols}{}
\usefonttheme{structurebold}
% make links like in solarized
\definecolor{base03}{RGB}{0,43,54}
\definecolor{base02}{RGB}{7,54,66}
\definecolor{base01}{RGB}{88,110,117}
\definecolor{base00}{RGB}{101,123,131}
\definecolor{base0}{RGB}{131,148,150}
\definecolor{base1}{RGB}{147,161,161}
\definecolor{base2}{RGB}{238,232,213}
\definecolor{base3}{RGB}{253,246,227}
\definecolor{yellow}{RGB}{181,137, 0}
\definecolor{orange}{RGB}{203,75,22}
\definecolor{red}{RGB}{220,50,47}
\definecolor{magenta}{RGB}{211,54,130}
\definecolor{violet}{RGB}{108,113,196}
\definecolor{blue}{RGB}{38,139,210}
\definecolor{cyan}{RGB}{42,161,152}
\definecolor{green}{RGB}{133,153,0}

\hypersetup{linkcolor=cyan, urlcolor=magenta}
\usecolortheme[named=cyan]{structure}
\setbeamercolor{example}{fg=green}
\setbeamercolor{block title example}{fg=green, bg=base3}
\setbeamercolor{block title}{bg=base3}
\usetheme{esc}

\usepackage{minted}
\usemintedstyle{solarized}
\newmintedfile{py}{fontsize=\footnotesize, xleftmargin=12pt}
\newmintedfile{pycon}{fontsize=\footnotesize, xleftmargin=12pt}

\newminted{py}{fontsize=\footnotesize, xleftmargin=12pt}
\newminted{pycon}{fontsize=\footnotesize, xleftmargin=12pt}
\newminted{console}{fontsize=\footnotesize, xleftmargin=12pt}
\newminted{latex}{fontsize=\footnotesize, xleftmargin=12pt}
\newminted{moin}{fontsize=\footnotesize, xleftmargin=12pt}

\usepackage{listings}
\lstdefinestyle{basic}{
    captionpos=t,%
    basicstyle=\color{black}\footnotesize\tt,%
    showspaces=false,%
    showstringspaces=false,%
    showtabs=false,%
    %
    keywordstyle=\color{green},%
    identifierstyle=\color{orange},%
    commentstyle=\color{base0}\it,%
    stringstyle=\color{cyan}%
}

\input{creative_commons/cc_beamer}

\author{Valentin Haenel}
\institute{Freelance Consultant and Software Developer \\
\url{http://haenel.co}}

\title{wiki2beamer}
\definecolor{gray}{rgb}{0.10,0.10,0.10}
\date{10 Apr 2013}

\begin{document}

\begin{frame}
    \titlepage
    \begin{center}
    \tiny{Version: \input{git-sha}}
    \hspace{1em}
    \tiny\url{https://github.com/esc/w2b-talk-de}
    \begin{tabular}[t]{lr}
        \mbox{\CcGroupBySa{0.35}{0.95ex}}  &
        \parbox[b]{8cm}{{\tiny\CcNote{\CcLongnameBySa}}} \\
    \end{tabular}
\end{center}\end{frame}

\input{slides.wiki.tex}

\end{document}
